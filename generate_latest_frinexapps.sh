...
if ! grep -q $(hostname) config/publish.properties; then 
    ...
    if docker build --no-cache -f docker/frinexapps.Dockerfile -t frinexapps:latest .
    then
        ...
        docker run --rm -v m2Directory:/maven/.m2/ -w /ExperimentTemplate frinexapps:latest /bin/bash -c "mvn install -gs /maven/.m2/settings.xml"
