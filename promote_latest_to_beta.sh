...
if ! grep -q $(hostname) config/publish.properties; then 
    ...
    docker run --rm -v buildServerTarget:/FrinexBuildService/artifacts -w /ExperimentTemplate/gwt-cordova frinexapps:beta /bin/bash -c "cp /ExperimentTemplate/ExperimentDesigner/changes /FrinexBuildService/artifacts/betachanges.txt"
    docker run --rm -v buildServerTarget:/FrinexBuildService/artifacts -w /FrinexBuildService frinexbuild:beta /bin/bash -c "chown frinex:daemon /FrinexBuildService/artifacts/*.xsd; chown frinex:daemon /FrinexBuildService/artifacts/*.html; chown frinex:daemon /FrinexBuildService/artifacts/*.txt;"
fi;
