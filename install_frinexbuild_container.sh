...
if ! grep -q $(hostname) publish.properties; then 
    ...
    docker run  -v gitCheckedout:/FrinexBuildService/git-checkedout -v gitRepositories:/FrinexBuildService/git-repositories -v incomingDirectory:/FrinexBuildService/incoming -v listingDirectory:/FrinexBuildService/listing -v processingDirectory:/FrinexBuildService/processing -v buildServerTarget:/FrinexBuildService/artifacts -v $workingDir/BackupFiles:/BackupFiles --rm -it --name frinexbuild-permissions frinexbuild:latest bash -c \
