...
if ! grep -q $(hostname) config/publish.properties; then 
    ...
    # docker run --restart unless-stopped --name frinex_listing_provider -e DOCKER_HOST="unix:///var/docker_frinex_build/docker.sock" -v /var/docker_frinex_build:/var/docker_frinex_build -d -p 8010:80 frinex_listing_provider:latest
