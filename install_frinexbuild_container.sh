...
if ! grep -q $(hostname) publish.properties; then 
    ...
    docker run --restart unless-stopped -v /var/run/docker.sock:/var/run/docker.sock -v gitCheckedout:/FrinexBuildService/git-checkedout -v gitRepositories:/FrinexBuildService/git-repositories -v webappsTomcatStaging:/usr/local/tomcat/webapps -v incomingDirectory:/FrinexBuildService/incoming -v listingDirectory:/FrinexBuildService/listing -v processingDirectory:/FrinexBuildService/processing -v buildServerTarget:/usr/local/apache2/htdocs -v $workingDir/BackupFiles:/BackupFiles -dit --name frinexbuild  -p 80:80 -p 8070:80 frinexbuild:latest
