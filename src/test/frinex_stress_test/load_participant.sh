...
for i in {1..20}; do
    ...
    # Participant: 20 rows
    # Participant: 16 rows
    curl --write-out %{http_code} --silent --show-error --output /dev/null -k -H 'Accept-Language: es' -d '[{"userId": "'$currentUserId'"}]' -H 'Content-Type: application/json' $currentUrl/metadata
done
for i in {1..27}; do
    # RAN and Verbal fluency for one participant:
    # ScreenData: 27 rows
...
for i in {1..35}; do
    ...
    # 11799 / 493 = 23
    # 1262 / 75 = 17
    curl --silent --show-error --output /dev/null -k -d '[{"tagDate" : "'$(date +"%Y-%m-%dT%H:%M:%S.000+0100")'","experimentName": "hammer_server","userId": "'$currentUserId'","screenName": "hammer_server","eventTag": "hammer_server","tagValue": "hammer_server","eventMs": "0"},{"tagDate" : "'$(date +"%Y-%m-%dT%H:%M:%S.000+0100")'","experimentName": "hammer_server","userId": "'$currentUserId'","screenName": "hammer_server","eventTag": "hammer_server","tagValue": "hammer_server","eventMs": "0"}]' -H 'Content-Type: application/json' $currentUrl/tagEvent
done
for i in {1..25}; do
    # RAN and Verbal fluency for one participant:
    # TagPairData: 25 rows
    ...
    # 83038 / 493 = 168
    # 6570 / 75 = 88
    curl --silent --show-error --output /dev/null -k -d '[{"tagDate" : "'$(date +"%Y-%m-%dT%H:%M:%S.000+0100")'","experimentName": "hammer_server","userId": "'$currentUserId'","screenName": "hammer_server","dataChannel": 0,"eventTag": "hammer_server","tagValue1": "hammer_server","tagValue2": "hammer_server","eventMs": "8999"},{"tagDate" : "'$(date +"%Y-%m-%dT%H:%M:%S.000+0100")'","experimentName": "hammer_server","userId": "'$currentUserId'","screenName": "hammer_server","dataChannel": 0,"eventTag": "hammer_server","tagValue1": "hammer_server","tagValue2": "hammer_server","eventMs": "8999"}]' -H 'Content-Type: application/json' $currentUrl/tagPairEvent
done
for i in {1..276}; do
    # RAN and Verbal fluency for one participant:
    # TimeStamp: 276 rows
    ...
    # 202988 / 493 = 412
    # 13216 / 75 = 177
    curl --silent --show-error --output /dev/null -k -d '[{"tagDate" : "'$(date +"%Y-%m-%dT%H:%M:%S.000+0100")'","experimentName": "hammer_server","userId": "'$currentUserId'","eventTag": "With Stimuli Screen","eventMs": "6"},{"tagDate" : "'$(date +"%Y-%m-%dT%H:%M:%S.000+0100")'","experimentName": "hammer_server","userId": "'$currentUserId'","eventTag": "hammer_server","eventMs": "2999"}]' -H 'Content-Type: application/json' $currentUrl/timeStamp
done
for i in {1..12}; do
    # RAN and Verbal fluency for one participant:
...
for i in {1..60}; do # audioA-30s.wav
    # RAN and Verbal fluency for one participant:
    # MediaData: 12 rows
    # MediaData: 4 rows
