...
RUN cd /FrinexBuildService/; npm install properties-reader; npm install check-disk-space; npm install got; npm install omgopass; npm install ssl-checker; npm install ini
...
RUN chown -R frinex:www-data /FrinexBuildService/script/build_admin_war.sh
