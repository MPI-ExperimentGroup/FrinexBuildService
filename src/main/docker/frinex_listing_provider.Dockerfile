...
RUN serviceOptions$(grep serviceOptions /FrinexBuildService/publish.properties | sed "s/serviceOptions[ ]*=[ ]*//g"); sed -i "s/DOCKER_SERVICE_OPTIONS/"$serviceOptions"/g" cgi/frinex_restart_experient.cgi
RUN dockerRegistry=$(grep dockerRegistry /FrinexBuildService/publish.properties | sed "s/dockerRegistry[ ]*=[ ]*//g"); sed -i "s/DOCKER_REGISTRY/"$dockerRegistry"/g" cgi/frinex_restart_experient.cgi
