...
RUN echo 'www-data ALL=(ALL) NOPASSWD: /usr/bin/docker service create --name ^[a-z0-9-_]+(_staging_web|_staging_admin|_production_web|_production_admin)(:space:--[a-z-]+=[a-z0-9.]+)*:space:-d:space:-p:space:8080:space:[a-zA-Z0-9-_.]+/[a-z0-9-_]+(_staging_web|_staging_admin|_production_web|_production_admin)\:stable$' >> /etc/sudoers
