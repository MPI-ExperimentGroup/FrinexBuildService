...
RUN cd /ExperimentTemplate \
    && sed -i '/frinex-parent/{n;s/POSTGRESQL/H2/}' /ExperimentDesigner/src/main/resources/application.properties \
    && sed -i '/frinex-parent/{n;s/PostgreSQLDialect/H2Dialect/}' /ExperimentDesigner/src/main/resources/application.properties \
    && sed -i '/frinex-parent/{n;s/spring.jpa.show-sql=false/spring.jpa.show-sql=true/}' /ExperimentDesigner/src/main/resources/application.properties
