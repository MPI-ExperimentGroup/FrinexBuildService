...
psql -h DatabaseStagingUrl -p DatabaseStagingPort -U frinex_staging_user -d postgres --no-align -t -c "select datname from pg_database where datistemplate = false and datname != 'postgres'" | while read -a currentexperiment ; do
    ...
    PGPASSWORD=examplechangethis psql -h DatabaseStagingUrl -p DatabaseStagingPort -U ${currentExperimentUser} -d $currentexperiment --no-align -t -c "select 'totalMediaResponses.value ' || select count (id) from audio_data" >> $outputFile;
