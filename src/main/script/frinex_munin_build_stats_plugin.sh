...
output_values() {
        ...
        { load_build_stats $1 > $dataDirectory/build_stats_$1.tmp; mv -f $dataDirectory/build_stats_$1.tmp $dataDirectory/build_stats_$1 }&
}

load_build_stats() {
    curl --silent -H 'Content-Type: application/json' http://$1/buildstats.json | grep -v '}' | grep -v '{' | sed 's/"//g' | sed 's/,//g' | sed 's/ //g' | sed "s/:/_$1.value /g"
}
