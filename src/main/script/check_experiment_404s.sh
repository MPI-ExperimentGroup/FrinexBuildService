...
possibleExperiment404s=$(sudo grep -E "GET /[a-z][a-z0-9_]{3,}/.*/.* 404" /var/log/tomcat/localhost_access_log.$(date +%F).txt | grep -v "/actuator/health" | cut -d / -f 4 | grep -E "^[a-z][a-z0-9_]{3,}$" | sort | uniq | paste -sd "|")
