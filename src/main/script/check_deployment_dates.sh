...
# sudo sudo touch /srv/tomcat/webapps/ROOT/known_sleepers.json.tmp
# sudo chown tomcat:tomcat /srv/tomcat/webapps/ROOT/known_sleepers.json.tmp
# sudo chmod a+w /srv/tomcat/webapps/ROOT/known_sleepers.json.tmp
echo "{" > /srv/tomcat/webapps/ROOT/known_sleepers.json.tmp
find /srv/tomcat/webapps/ -maxdepth 1 -mindepth 1 -type f -name *-admin.war.disabled -printf '"%f,' | sed "s/-admin.war.disabled/\"/g" | sed 's/,$/\n/' | sed 's/,/,\n/g' >> /srv/tomcat/webapps/ROOT/known_sleepers.json.tmp
echo "}" >> /srv/tomcat/webapps/ROOT/known_sleepers.json.tmp
echo "{" > /srv/tomcat/webapps/ROOT/running_experiments.json.tmp
find /srv/tomcat/webapps/ -maxdepth 1 -mindepth 1 -type f -name *-admin.war -printf '"%f,' | sed "s/-admin.war/\"/g" | sed 's/,$/\n/' | sed 's/,/,\n/g' >> /srv/tomcat/webapps/ROOT/running_experiments.json.tmp
echo "}" >> /srv/tomcat/webapps/ROOT/running_experiments.json.tmp

mv -f /srv/tomcat/webapps/ROOT/known_sleepers.json.tmp /srv/tomcat/webapps/ROOT/known_sleepers.json
mv -f /srv/tomcat/webapps/ROOT/running_experiments.json.tmp /srv/tomcat/webapps/ROOT/running_experiments.json
