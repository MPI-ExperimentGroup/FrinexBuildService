    ...
    if [[ ${#appNameInternal} -gt 2 ]] ; then
        ...
        if [ "$(psql -h DatabaseStagingUrl -p DatabaseStagingPort -U db_manager_frinex_staging -d postgres -tAc "SELECT 1 FROM pg_database WHERE datname='frinex_${appNameInternal}_db'" )" = '1' ]; then
            ...
            for table in "${tablesToUpdate[@]}"; do
                ...
                is_identity=$(psql -h DatabaseStagingUrl -p DatabaseStagingPort -U db_manager_frinex_staging -d "frinex_${appNameInternal}_db" -tAc "
                SELECT is_identity
                ...
                if [[ "$is_identity" == "NO" ]]; then
                    psql -h DatabaseStagingUrl -p DatabaseStagingPort -U db_manager_frinex_staging -d "frinex_${appNameInternal}_db" -c \
