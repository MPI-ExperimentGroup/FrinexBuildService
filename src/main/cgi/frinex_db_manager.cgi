...
if [[ "$QUERY_STRING" =~ ^frinex_[a-z0-9_]*_db$ ]]; then
    ...
    if [[ ${#appNameInternal} -gt 2 ]] ; then
        ...
        if [ "$(psql -h DatabaseStagingUrl -p DatabaseStagingPort -U db_manager_frinex_staging -d postgres -tAc "SELECT 1 FROM pg_database WHERE datname='frinex_${appNameInternal}_db'" )" = '1' ]; then
            ...
            for table in "${tablesToUpdate[@]}"; do
                ...
                if [[ "$is_identity" == "NO" ]]; then
                    alterResult=$(PGPASSWORD='DatabaseStagingPass' psql -h DatabaseStagingUrl -p DatabaseStagingPort -U frinex_${appNameInternal}_user -d "frinex_${appNameInternal}_db" -tAc \
                        "ALTER TABLE $table ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY;")
                    setValResult=$(PGPASSWORD='DatabaseStagingPass' psql -h DatabaseStagingUrl -p DatabaseStagingPort -U frinex_${appNameInternal}_user -d "frinex_${appNameInternal}_db" -tAc \
                        "SELECT setval(pg_get_serial_sequence('$table','id'), GREATEST(COALESCE((SELECT MAX(id) FROM $table),0),1),true);")
                    messageString="$messageString\n$alterResult\n$setValResult\n"
                fi
            done
        else
        ...
        if [ "$(psql -h DatabaseProductionUrl -p DatabaseProductionPort -U db_manager_frinex_production -d postgres -tAc "SELECT 1 FROM pg_database WHERE datname='frinex_${appNameInternal}_db'" )" = '1' ]; then
            ...
            tablesToUpdate=("data_deletion_log" "event_time" "group_data" "audio_data" "screen_data" "stimulus_response" "tag_data" "tag_pair_data" "time_stamp" "participant")
            for table in "${tablesToUpdate[@]}"; do
                is_identity=$(PGPASSWORD='DatabaseProductionPass' psql -h DatabaseProductionUrl -p DatabaseProductionPort -U frinex_${appNameInternal}_user -d "frinex_${appNameInternal}_db" -tAc "
                SELECT is_identity
                FROM information_schema.columns
                WHERE table_schema = 'public'
                AND table_name = '$table'
                ...
                is_identity=$(echo "$is_identity" | xargs)
                if [[ "$is_identity" == "NO" ]]; then
                    alterResult=$(PGPASSWORD='DatabaseProductionPass' psql -h DatabaseProductionUrl -p DatabaseProductionPort -U frinex_${appNameInternal}_user -d "frinex_${appNameInternal}_db" -tAc \
                        "ALTER TABLE $table ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY;")
                    setValResult=$(PGPASSWORD='DatabaseProductionPass' psql -h DatabaseProductionUrl -p DatabaseProductionPort -U frinex_${appNameInternal}_user -d "frinex_${appNameInternal}_db" -tAc \
                        "SELECT setval(pg_get_serial_sequence('$table','id'), GREATEST(COALESCE((SELECT MAX(id) FROM $table),0),1),true);")
                    messageString="$messageString\n$alterResult\n$setValResult\n"
                fi
            done
