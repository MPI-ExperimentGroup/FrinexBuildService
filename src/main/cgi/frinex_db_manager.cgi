    ...
    if [[ ${#appNameInternal} -gt 2 ]] ; then
        ...
        if [ "$(psql -h DatabaseStagingUrl -p DatabaseStagingPort -U db_manager_frinex_staging -d postgres -tAc "SELECT 1 FROM pg_database WHERE datname='frinex_${appNameInternal}_db'" )" = '1' ]; then
            ...
            for table in "${tablesToUpdate[@]}"; do
                ...
                if [[ "$is_identity" == "NO" ]]; then
                    alterResult=$(PGPASSWORD='DatabaseStagingPass' psql -h DatabaseStagingUrl -p DatabaseStagingPort -U frinex_${appNameInternal}_user -d "frinex_${appNameInternal}_db" -c \
                        "ALTER TABLE $table ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY;")
                    setValResult=$(PGPASSWORD='DatabaseStagingPass' psql -h DatabaseStagingUrl -p DatabaseStagingPort -U frinex_${appNameInternal}_user -d "frinex_${appNameInternal}_db" -c \
        ...
        if [ "$(psql -h DatabaseProductionUrl -p DatabaseProductionPort -U db_manager_frinex_production -d postgres -tAc "SELECT 1 FROM pg_database WHERE datname='frinex_${appNameInternal}_db'" )" = '1' ]; then
            ...
            for table in "${tablesToUpdate[@]}"; do
                ...
                is_identity=$(echo "$is_identity" | xargs)
                if [[ "$is_identity" == "NO" ]]; then
                    alterResult=$(PGPASSWORD='DatabaseProductionPass' psql -h DatabaseProductionUrl -p DatabaseProductionPort -U frinex_${appNameInternal}_user -d "frinex_${appNameInternal}_db" -c \
                        "ALTER TABLE $table ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY;")
                    setValResult=$(PGPASSWORD='DatabaseProductionPass' psql -h DatabaseProductionUrl -p DatabaseProductionPort -U frinex_${appNameInternal}_user -d "frinex_${appNameInternal}_db" -c \
                        "SELECT setval(pg_get_serial_sequence('$table','id'), GREATEST(COALESCE((SELECT MAX(id) FROM $table),0),1),true);")
                    messageString="$messageString\n$alterResult\n$setValResult\n"
                fi
            done
