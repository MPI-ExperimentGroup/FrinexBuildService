...
jettyMax=$(echo "$QUERY_STRING" | sed -n 's/.*jettyMax=\([^&]*\).*/\1/p')
jettyJobs=$(echo "$QUERY_STRING" | sed -n 's/.*jettyJobs=\([^&]*\).*/\1/p')
gcOverhead=$(echo "$QUERY_STRING" | sed -n 's/.*gcOverhead=\([^&]*\).*/\1/p')
...
(
    flock -n 200 || exit 1
    echo "date,maxInstances,instanceCount,avgMs,requests,service,container,status,cpuUsage,jvmMemoryUsed,jvmMemoryMax,threadsBusy,dbActive,dbIdle,jettyMax,jettyJobs,gcOverhead,dbMax" > $targetDir/request_scaling.temp
    echo "$(date +'%Y-%m-%d %H:%M:%S'),$maxInstances,$instanceCount,$avgMs,$total,$serviceName,$container,$status,$cpuUsage,$jvmMemoryUsed,$jvmMemoryMax,$threadsBusy,$dbActive,$dbIdle,$jettyMax,$jettyJobs,$gcOverhead,$dbMax" >> $targetDir/request_scaling.temp
        ...
        if (( avgMs > 250 )); then
            ...
            # else
                if (( instanceCount < maxInstances )); then
                    ...
                    # todo: the first instance is started with DOCKER_SERVICE_OPTIONS and subsequent instances are started with DOCKER_SCALED_OPTIONS
