...
if ! grep -q $(hostname) config/publish.properties; then 
    ...
    then 
        ...
        docker run --rm -v m2Directory:/maven/.m2/ -w /ExperimentTemplate frinexapps-jdk:snapshot /bin/bash -c "mvn install -pl :parent,:common,:frinex-experiment-designer -gs /maven/.m2/settings.xml"
