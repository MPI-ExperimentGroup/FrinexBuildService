...
if ! grep -q $(hostname) config/publish.properties; then 
    ...
    #docker run -v buildServerTarget:/FrinexBuildService/artifacts -it --rm --name bootstrap_public_stats frinex_db_manager:latest bash -c "/FrinexBuildService/stats/bootstrap_florians_userids_statistics.sh > /FrinexBuildService/artifacts/bootstrap_florians_userids_statistics.json"
    ...
    cat stats/productionotherpre2022-08-26_public_stats.json | docker run -v buildServerTarget:/FrinexBuildService/artifacts --rm  -i --name productionBQ4pre2022-08-26_public_stats frinexbuild:latest /bin/bash -c 'cat > /FrinexBuildService/artifacts/productionBQ4pre2022-08-26_public_stats.json'
    ...
    # also search for experiment that mention power supply or reload page because these warnings need to be made clear to the participants for more accurate timing
    docker run -v buildServerTarget:/FrinexBuildService/artifacts --rm -it  -v buildServerTarget:/FrinexBuildService/artifacts --name frinex_usage_stats frinexbuild:latest bash -c "echo '' > /FrinexBuildService/artifacts/frinex_usage_stats.json; for frinexElement in 'addMediaTrigger' 'addRecorderDtmfTrigger' 'addTimerTrigger' 'evaluatePause' 'pauseMedia' 'playMedia' 'resetTrigger' 'startFrameRateTimer' 'startTimer' 'triggerDefinition' 'triggerMatching' 'addFrameTimeTrigger' 'pause' 'adaptivevocabularyassessment' 'AdVocAsStimuliProvider' 'power' 'supply' 'reload' 'refresh'; do echo \"\\\"\$frinexElement\\\": {\" >> /FrinexBuildService/artifacts/frinex_usage_stats.json; grep -c \"<\$frinexElement \" artifacts/*/*.xml | grep -v ':0$' | sed 's|artifacts/|}, {\\\"repository\\\": \\\"|g' | sed 's|/|\", \"experiment\": \"|g' | sed 's|.xml:|\", \"feature\": \"'\$frinexElement'\", \"count\": |g' >> /FrinexBuildService/artifacts/frinex_usage_stats.json; echo '},' >> /FrinexBuildService/artifacts/frinex_usage_stats.json; done;"
    # this XML element usage stats has been changed to use the XML from the artifacts directory because the gitCheckedout volume has been discarded
