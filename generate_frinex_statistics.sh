...
if ! grep -q $(hostname) config/publish.properties; then 
    ...
    cat $(dirname "$0")/src/main/stats/productionother_public_stats.json | docker run -v buildServerTarget:/FrinexBuildService/artifacts --rm  -i --name productionother_public_stats frinexbuild:latest /bin/bash -c 'cat > /FrinexBuildService/artifacts/productionBQ4_public_stats.json'
    cat $(dirname "$0")/src/main/stats/production_public_stats.json | docker run -v buildServerTarget:/FrinexBuildService/artifacts --rm -i --name production_public_stats frinexbuild:latest /bin/bash -c 'cat > /FrinexBuildService/artifacts/production_public_stats.json'
