...
if ! grep -q $(hostname) config/publish.properties; then 
    ...
    docker run -v buildServerTarget:/FrinexBuildService/artifacts -v gitCheckedout:/FrinexBuildService/git-checkedout --rm -it --name frinex_usage_stats frinexbuild:latest bash -c "echo '' > /FrinexBuildService/artifacts/frinex_usage_stats.json; for frinexElement in '<addMediaTrigger' '<addRecorderDtmfTrigger' '<addTimerTrigger' '<evaluatePause' '<pauseMedia' '<playMedia' '<resetTrigger' '<startFrameRateTimer' '<startTimer' '<triggerDefinition' '<triggerMatching' '<addFrameTimeTrigger' '<pause'; do echo \"\\\"\$frinexElement\\\": {\" >> /FrinexBuildService/artifacts/frinex_usage_stats.json; grep -c \$frinexElement git-checkedout/*/*.xml | grep -v ':0$' | sed 's|git-checkedout/|}, {\\\"repository\\\": \\\"|g' | sed 's|/|\\\", \\\"experiment\\\": \\\"|g' | sed 's|.xml:|\\\", \\\"usage\\\": |g' >> /FrinexBuildService/artifacts/frinex_usage_stats.json; echo '}' >> /FrinexBuildService/artifacts/frinex_usage_stats.json; done;"
fi;
