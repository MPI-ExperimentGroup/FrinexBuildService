...
if ! grep -q $(hostname) config/publish.properties; then 
    ...
    cat $workingDir/src/main/stats/productionother_public_stats.json | docker run -v buildServerTarget:/FrinexBuildService/artifacts -i frinexbuild:latest /bin/bash -c 'cat > /FrinexBuildService/artifacts/productionBQ4_public_stats.json'
    cat $workingDir/src/main/stats/production_public_stats.json | docker run -v buildServerTarget:/FrinexBuildService/artifacts -i frinexbuild:latest /bin/bash -c 'cat > /FrinexBuildService/artifacts/production_public_stats.json'

    # generate the XML element usage stats file
    docker run -v buildServerTarget:/FrinexBuildService/artifacts -v gitCheckedout:/FrinexBuildService/git-checkedout --rm -it --name frinex_usage_stats frinexbuild:latest bash -c "echo '' > /FrinexBuildService/artifacts/frinex_usage_stats.json; for frinexElement in 'addMediaTrigger' 'addRecorderDtmfTrigger' 'addTimerTrigger' 'evaluatePause' 'pauseMedia' 'playMedia' 'resetTrigger' 'startFrameRateTimer' 'startTimer' 'triggerDefinition' 'triggerMatching' 'addFrameTimeTrigger' 'pause'; do echo \"\\\"\$frinexElement\\\": {\" >> /FrinexBuildService/artifacts/frinex_usage_stats.json; grep -c \"<\$frinexElement \" git-checkedout/*/*.xml | grep -v ':0$' | sed 's|git-checkedout/|}, {\\\"repository\\\": \\\"|g' | sed 's|/|\", \"experiment\": \"|g' | sed 's|.xml:|\", \"feature\": \"'\$frinexElement'\", \"count\": |g' >> /FrinexBuildService/artifacts/frinex_usage_stats.json; echo '},' >> /FrinexBuildService/artifacts/frinex_usage_stats.json; done;"

    # subset the XML element usage stats file for BQ4 session 1 and session 2
    docker run -v buildServerTarget:/FrinexBuildService/artifacts--rm -it --name frinex_usage_stats frinexbuild:latest bash -c "cat /FrinexBuildService/artifacts/frinex_usage_stats.json | grep -E \"ausimplereactiontime_bq4_timestudy|visimplereactiontime_bq4_timestudy|picturenaming_bq4_timestudy|sentencegeneration_bq4_timestudy|sentencemonitoring_bq4_timestudy|werkwoorden_bq4_timestudy|s3ausimplereactiontime|s2visimplereactiontime|s2picturenaming|s3sentencegeneration|s3sentencemonitoring|s2werkwoorden|mpiausimplereactiontime|mpivisimplereactiontime|mpipicturenaming|mpisentencegeneration|mpisentencemonitoring|mpiwerkwoorden\" > /FrinexBuildService/artifacts/frinex_BQ4_usage_stats.json;"
fi;
