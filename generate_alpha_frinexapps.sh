...
if ! grep -q $(hostname) config/publish.properties; then 
    ...
    then 
        ...
        # make sure the local .m2 directory has the alpha jar files. In this case only frinex common and the parent pom are required, compiling the GWT component is not needed here
        ...
        then
            ...
            #, pulling upstream dependency for the next run"
