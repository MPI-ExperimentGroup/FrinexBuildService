...
if ! grep -q $(hostname) src/main/config/publish.properties; then 
    ...
    docker run --rm -v gitCheckedout:/FrinexBuildService/git-checkedout -v gitRepositories:/FrinexBuildService/git-repositories -v buildServerTarget:/FrinexBuildService/artifacts -v $workingDir/BackupFiles:/BackupFiles -it --name frinexbuild-restore frinexbuild:latest /bin/bash -c "rsync -a /BackupFiles/buildartifacts/ /FrinexBuildService/artifacts/; rsync -a /BackupFiles/git-* /FrinexBuildService/; chown -R frinex:daemon /FrinexBuildService; chmod -R ug+rwx /FrinexBuildService; chown -R frinex:daemon /FrinexBuildService/artifacts; chmod -R ug+rwx /FrinexBuildService/artifacts;";
fi;