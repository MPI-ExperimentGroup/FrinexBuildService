...
if ! grep -q $(hostname) src/main/docker/publish.properties; then 
    ...
    docker run --rm -v gitCheckedout:/FrinexBuildService/git-checkedout -v gitRepositories:/FrinexBuildService/git-repositories -v buildServerTarget:/usr/local/apache2/htdocs -v $workingDir/BackupFiles:/BackupFiles -it --name frinexbuild-restore frinexbuild:latest /bin/bash -c "rsync -a /BackupFiles/buildartifacts/ /usr/local/apache2/htdocs/; rsync -a /BackupFiles/git-* /FrinexBuildService/; chown -R frinex:daemon /FrinexBuildService; chmod -R ug+rwx /FrinexBuildService;chown -R frinex:daemon /usr/local/apache2/htdocs; chmod -R ug+rwx /usr/local/apache2/htdocs;";
fi;