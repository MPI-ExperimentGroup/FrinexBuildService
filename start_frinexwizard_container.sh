...
if ! grep -q $(hostname) config/publish.properties; then 
    ...
    docker run --restart unless-stopped -dit --name frinexwizard -v WizardTemplates:/ExperimentTemplate/ExperimentDesigner/src/main/resources/static/compiled_templates/ -v buildServerTarget:/FrinexBuildService/artifacts -p 7070:8080 frinexwizard:latest
fi;
